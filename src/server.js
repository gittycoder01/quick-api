...
const methodNotAllowed = (operation, allowedMethod) => (req, res) =>
  res
    .status(405)
    .type(xmlType)
    ...
    .send(
      buildErrorResponse(
        'METHOD_NOT_ALLOWED',
        `${operation} only supports ${allowedMethod} requests.`,
      ),
    );
...
app.all(
  '/ips-payments/service-requests',
  methodNotAllowed('Service request', 'POST'),
);
...
app.all(
  '/ips-payments/health-checks',
  methodNotAllowed('Health check', 'PUT'),
);
...
app.all('/credit-status', methodNotAllowed('Credit status update', 'PUT'));
